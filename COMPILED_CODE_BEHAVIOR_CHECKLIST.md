...
- [x] Configure a clean Debug build with tests enabled:
      ...
      in `08.5-v1.0-closeout/evidence/gates/PROVENANCE.txt` at `d08b6e5`).
...
- [x] Run the full CTest suite:
      DONE. **335/335 (100% tests passed, 0 tests failed) in all three trees**
      at `d08b6e5` (`08.5-v1.0-closeout/evidence/gates/PROVENANCE.txt`); full
...
- [x] Confirm the expected discovered test surface is present. The tree contains
      **333 Catch2 test cases** across 23 files, of which **332 are registered
      with ctest** (`Total Tests: 335` including the `preflight`, `start_xvfb`
      and `stop_xvfb` fixture tests). RECOMPUTED at `d08b6e5`, not transcribed.
      The arithmetic: 333 source cases − 1 hidden case = 332 registered Catch2
      cases, + 3 fixture tests = 335 `Total Tests`. The one case not registered
...
```
      ...
      Per file, at the time of writing (plan 08.5-05, commit `d08b6e5`):
      ...
      | `test_wm_rules.cpp` | 16 | | `test_wm_resource.cpp` | 4 |
      ...
      `[rules]` cases, 17 -> 25"); `test_wm_rules.cpp` 8 -> 16 (+5 from plan
      ...
      drifts — plus 3 further cases from the Codex review-fixes round on PR #5,
...
- [x] Run a Release build and tests:
      ...
      100% tests passed, 0 tests failed out of 335. The release gate also runs
...
- [x] Run an AddressSanitizer/UBSan build. Treat use-after-free, invalid enum
      ...
      tests passed, 0 tests failed out of 335, "no sanitizer findings". The six
...
- [x] No ASan/UBSan reports in full automated and runtime smoke tests.
      ...
      passed, 0 tests failed out of 335, "no sanitizer findings". The six files
...
- [x] Commit hash and branch.
      ...
      `d08b6e5d3824822b823c0aea4971223ca51b0692` (`d08b6e5`), branch
...
- [x] Full `ctest --output-on-failure` logs.
      ...
      failed out of 335 in every tree.
- [x] Runtime smoke transcript with `xprop -root` and `xwininfo -root -tree`
      ...
      plus one `xmessage` client titled `wm2-smoke-d08b6e5`, all three stopped
      by PID.
